...
must_haves:
  truths:
    ...
    - "data/packages/ and data/dotfiles/ directories exist with topic-centric layout"
    - "data/dotfiles/ has git/, zsh/, bash/ subdirectories per CONTEXT.md"
    - "packages.sh provides load_packages() function with DATA_DIR validation"
  artifacts:
...
<task type="auto">
  ...
  <action>
    1. Create new directory structure:
       ...
       - mkdir -p data/packages/profiles
       - mkdir -p data/dotfiles/git data/dotfiles/zsh data/dotfiles/bash  # topic-centric per CONTEXT.md
       - touch data/packages/.gitkeep data/packages/profiles/.gitkeep
       - touch data/dotfiles/git/.gitkeep data/dotfiles/zsh/.gitkeep data/dotfiles/bash/.gitkeep
...
<task type="auto">
  ...
  <action>
    ...
    3. load_packages() function:
       ...
       - VALIDATION: Check DATA_DIR exists, log error and return 1 if not
       - VALIDATION: Check file exists before reading, log error and return 1 if not
       - VALIDATION: Check file is readable, log error if permissions issue
       ...
       - Returns 0 on success, 1 if validation fails or file not found
...
- data/dotfiles/ exists with git/, zsh/, bash/ subdirectories (topic-centric layout)
- All shell scripts pass syntax validation
- load_packages() validates DATA_DIR before attempting to read files
</success_criteria>
