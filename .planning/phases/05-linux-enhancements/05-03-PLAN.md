...
must_haves:
  ...
  key_links:
    ...
    - from: "src/install/rust-cli.sh"
      ...
      via: "show_category_menu(), ask_tool()"
      pattern: "show_category_menu"
---
...
<task type="auto">
  <name>Task 0: Create src/core/interactive.sh (shared UI functions)</name>
  <files>src/core/interactive.sh</files>
  <action>
  Create `src/core/interactive.sh` with shared interactive selection functions used by rust-cli.sh, dev-env.sh, and ai-tools.sh:

  1. Header (shebang, description, NOTE about no set -e)
  2. Source guard: `[[ -n "${_INTERACTIVE_SOURCED:-}" ]] && return 0; readonly _INTERACTIVE_SOURCED=1`
  3. Source logging.sh if not already sourced (same pattern as errors.sh)

  4. `show_category_menu()` function:
     - Takes category name as $1 and description as $2
     - Shows: "Install $1? ($2)" with options 1) All 2) Choose 3) Skip
     - `read -rp "Select [1-3]: " choice`
     - Return 0=all, 1=choose, 2=skip
     - In NONINTERACTIVE mode: return 0 (install all)

  5. `ask_tool()` function for individual selection:
     - Takes tool name as $1
     - `read -rp "Install $1? [Y/n]: " answer`
     - Return 0 (yes) unless answer starts with n/N

  6. Export functions: `export -f show_category_menu ask_tool`
  </action>
  <verify>Run `bash -n src/core/interactive.sh` -- must pass syntax check</verify>
  <done>src/core/interactive.sh provides shared show_category_menu() and ask_tool() for all cross-platform installers</done>
</task>
...
<task type="auto">
  ...
  3. Source core utilities from `${SCRIPT_DIR}/../core/` (one level up, since src/install/ is sibling to src/core/):
     - logging.sh, idempotent.sh, errors.sh, packages.sh, platform.sh, **interactive.sh**
  ...
  5. `show_category_menu()` and `ask_tool()` are provided by `src/core/interactive.sh` (sourced in step 3).
