...
<task type="auto">
  ...
  3. Source core utilities from `${SCRIPT_DIR}/../core/`: logging.sh, idempotent.sh, errors.sh, packages.sh, **interactive.sh**

  4. `show_category_menu()` and `ask_tool()` are provided by `src/core/interactive.sh` (sourced in step 3).
     Do NOT define them locally — use the core versions directly.

  5. Add `install_ai_tool()` function:
     - First check if entry contains `:` — if NOT (no prefix, e.g. bare `uv`), log_debug "Skipping unprefixed entry: $entry" and return 0
     ...
     - Case on prefix:
       ...
       - `*)`: `log_debug "Skipping unknown prefix: $prefix for $tool"`, return 0

     NOTE: Entries without a recognized prefix (bare words like `uv`, or non-AI npm packages like `npm:bmad-method`) are silently skipped via log_debug. The installer only acts on `npm:` (AI CLI tools) and `curl:` entries. All other entries (npx:, uv:, bare words) are informational references, not installable targets.

  6. Add `offer_ollama_model()` function:
  ...
  9. Main section:
      ...
      - Pass ALL entries to install_ai_tool (it handles prefix filtering internally — skips npx:, uv:, bare words via log_debug)
      - If "choose" mode: ask_tool only for `npm:` and `curl:` entries (the installable ones), skip others silently
      - Install loop: call install_ai_tool for each selected entry, record_failure only on actual install failures (not skips)
