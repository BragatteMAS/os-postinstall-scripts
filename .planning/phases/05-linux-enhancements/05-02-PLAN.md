...
<task type="auto">
  ...
  4. Add `retry_with_backoff()` function (same as apt.sh):
     - 3 attempts, delays: 5s, 15s, 30s
     - Log retries with log_warn
...
<task type="auto">
  ...
  2. Add `retry_with_backoff()` (same implementation)
  ...
  4. Add `snap_install()`:
     ...
     - Try: `sudo snap install "$pkg" $classic_flag`
     - If fails AND no classic_flag was set: retry with `--classic` (auto-detect classic need)
...
- Both files contain `load_packages`, `retry_with_backoff`, `record_failure`
