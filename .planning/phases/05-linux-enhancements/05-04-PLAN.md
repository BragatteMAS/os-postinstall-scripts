...
<task type="auto">
  ...
  **fnm.sh** - Cross-platform fnm installer:
  ...
  2. Source guard + conditional SCRIPT_DIR (safe for sourcing by dev-env.sh):
     ```
  ...
  **uv.sh** - Cross-platform uv installer:
  ...
  1. Same header/sourcing pattern with source guard + conditional SCRIPT_DIR:
     ```
...
<task type="auto">
  ...
  3. Source core utilities from `${SCRIPT_DIR}/../core/`: logging.sh, errors.sh, **interactive.sh**
