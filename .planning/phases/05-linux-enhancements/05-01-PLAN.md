...
<task type="auto">
  ...
  1. Add `retry_with_backoff()` function after the source block, before APT helpers:
     - Takes a command as arguments: `retry_with_backoff cmd arg1 arg2...`
     - 3 attempts, delays: 5s, 15s, 30s (exponential backoff)
     - Log each retry: `log_warn "Retry ${attempt}/${max_attempts} in ${delay}s..."`
     - Return 0 on success, 1 after all attempts exhausted
...
- File contains `retry_with_backoff` function
