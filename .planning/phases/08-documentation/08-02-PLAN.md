...
<task type="auto">
  ...
  <action>
    ...
    **Structure:**
    ...
    4. **Project Structure**: Brief overview of src/core/, src/platforms/, src/install/ (cross-platform tool installers: rust-cli, fnm, uv, ai-tools, dev-env), src/installers/ (dotfiles-install.sh), data/packages/, data/dotfiles/, tests/
    ...
    8. **Pull Request Process**:
       ...
       - Run ShellCheck: `find src -name '*.sh' -exec shellcheck -x {} +` (ZERO warnings required)
    ...
    9. **ShellCheck**: REQUIRED for all shell script changes.
       ...
       - Run: `find src -name '*.sh' -exec shellcheck -x {} +` (note: `src/**/*.sh` requires `shopt -s globstar` which is not portable)
