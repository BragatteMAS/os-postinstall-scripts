...
must_haves:
  ...
  artifacts:
    - path: "scripts/utils/platform.sh"
  ...
  key_links:
    - from: "scripts/utils/platform.sh"
...
Output: `scripts/utils/platform.sh` with detect_platform() and verify_all() functions.
</objective>
...
<task type="auto">
  ...
  <files>scripts/utils/platform.sh</files>
  <action>
Create `scripts/utils/platform.sh` implementing:
...
Supported distros list: ubuntu debian pop linuxmint elementary zorin
  ...
  <verify>
Run: `bash -c 'source scripts/utils/platform.sh && detect_platform && echo "OS=$DETECTED_OS DISTRO=$DETECTED_DISTRO PKG=$DETECTED_PKG ARCH=$DETECTED_ARCH BASH=$DETECTED_BASH"'`
...
1. Source scripts/utils/platform.sh
...
Run this test manually and document output.
  ...
  <verify>
Run: `bash scripts/utils/platform.sh` (should not output anything when sourced)
Run: `bash -c 'source scripts/utils/platform.sh && verify_all'`
...
<verification>
1. `bash -c 'source scripts/utils/platform.sh'` - No errors on source
2. `bash -c 'source scripts/utils/platform.sh && detect_platform && [[ -n "$DETECTED_OS" ]]'` - Variables exported
3. `bash -c 'source scripts/utils/platform.sh && verify_bash_version'` - Returns 0 on Bash 4+
4. File does NOT contain `set -e`
</verification>

<success_criteria>
1. scripts/utils/platform.sh exists and is sourceable
