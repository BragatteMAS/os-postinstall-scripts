...
must_haves:
  ...
  artifacts:
    - path: "scripts/lib/idempotent.sh"
  ...
  key_links:
    - from: "scripts/lib/idempotent.sh"
      to: "command, grep, ln"
...
Output: `scripts/lib/idempotent.sh` with guard functions for safe repeated execution.
</objective>
...
<task type="auto">
  ...
  <files>scripts/lib/idempotent.sh</files>
  <action>
Create `scripts/lib/idempotent.sh` implementing:
...
Run test sequence:
```bash
source scripts/lib/idempotent.sh
# Test is_installed
...
<verification>
1. `bash -c 'source scripts/lib/idempotent.sh'` - No errors on source
2. `bash -c 'source scripts/lib/idempotent.sh && is_installed bash'` - Returns 0
3. `bash -c 'source scripts/lib/idempotent.sh && is_installed nonexistent_xyz'` - Returns 1
4. File does NOT contain `set -e`
</verification>

<success_criteria>
1. scripts/lib/idempotent.sh exists and is sourceable
